...
func (tpl *Template) Send(msg *TemplateMessage) (msgID int64, err error) {
	...
	if result.ErrCode != 0 {
		return 0, fmt.Errorf("template msg send error : errcode=%v , errmsg=%v", result.ErrCode, result.ErrMsg)
	}
	msgID = result.MsgID
	return msgID, nil
}
...
func SendTemplateMsg(msgData map[string]*TemplateDataItem, openid string, templateID string, url string) (int64, error) {
	template := NewTemplate(WechatOfficialAccount.GetContext())
	msg := &TemplateMessage{
		ToUser:     openid,     //"o8XSO6KtEaHu8Q_tDKctnHTnJBBo",
		TemplateID: templateID, //"OPcCnhuyujvm2QeGAZieCy18hkuVJTsTGJJo0FoZgFI",
