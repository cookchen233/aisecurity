...
func (uq *UserQuery) QueryUpdater() *AdminQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUpdater: uq.withUpdater.Clone(),
...
func (uq *UserQuery) WithUpdater(opts ...func(*AdminQuery)) *UserQuery {
	query := (&AdminClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withUpdater = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withUpdater; query != nil {
		if err := uq.loadUpdater(ctx, query, nodes, nil,
			func(n *User, e *Admin) { n.Edges.Updater = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadUpdater(ctx context.Context, query *AdminQuery, nodes []*User, init func(*User), assign func(*User, *Admin)) error {
...
func (uq *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := uq.ctx.Fields; len(fields) > 0 {
		...
		if uq.withUpdater != nil {
			_spec.Node.AddColumnOnce(user.FieldUpdaterID)
		}
	}
