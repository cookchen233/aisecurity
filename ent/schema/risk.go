...
func (Risk) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("updater", Admin.Type).Ref("risk_updater").Field("updated_by").Required().Unique(),
		...
		edge.From("reporter", Employee.Type).Ref("risk_reporter").Field("reporter_id").Unique().Required(),
