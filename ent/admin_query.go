...
type AdminQuery struct {
	...
	withRiskReportingAdmin *RiskReportingQuery
...
// QueryRiskReportingAdmin chains the current query on the "RiskReportingAdmin" edge.
func (aq *AdminQuery) QueryRiskReportingAdmin() *RiskReportingQuery {
	query := (&RiskReportingClient{config: aq.config}).Query()
...
func (aq *AdminQuery) Clone() *AdminQuery {
	...
	return &AdminQuery{
		...
		withRiskReportingAdmin: aq.withRiskReportingAdmin.Clone(),
...
// WithRiskReportingAdmin tells the query-builder to eager-load the nodes that are connected to
// the "RiskReportingAdmin" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AdminQuery) WithRiskReportingAdmin(opts ...func(*RiskReportingQuery)) *AdminQuery {
	query := (&RiskReportingClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withRiskReportingAdmin = query
	return aq
}
...
func (aq *AdminQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Admin, error) {
	...
	if query := aq.withRiskReportingAdmin; query != nil {
		if err := aq.loadRiskReportingAdmin(ctx, query, nodes,
			func(n *Admin) { n.Edges.RiskReportingAdmin = []*RiskReporting{} },
			func(n *Admin, e *RiskReporting) { n.Edges.RiskReportingAdmin = append(n.Edges.RiskReportingAdmin, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AdminQuery) loadRiskReportingAdmin(ctx context.Context, query *RiskReportingQuery, nodes []*Admin, init func(*Admin), assign func(*Admin, *RiskReporting)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(riskreporting.FieldCreatedBy)
	}
	query.Where(predicate.RiskReporting(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(admin.RiskReportingAdminColumn), fks...))
	}))
